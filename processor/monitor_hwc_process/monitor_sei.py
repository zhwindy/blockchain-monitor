...
def get_data():
    ...
    cursor = conn.cursor()
    sql = "select block_number, timestamp from sei_source_block order by block_number desc limit 1"
    info = {
        "block_number": 0,
        "create_time": 0
    }
...
def monitor():
    ...
    if int(diff_min) > THRESHOLD:
        alert(text)
        send_email_alert("SEI", block_number, diff_min)
