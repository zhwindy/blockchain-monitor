...
def get_data():
    ...
    cursor = conn.cursor()
    sql = "select block_number, timestamp from cro_source_block order by block_number desc limit 1"
    info = {
        "block_number": 0,
        "create_time": 0
    }
...
def cro_monitor():
    ...
    if int(diff_min) > THRESHOLD:
        alert(text)
        send_email_alert("CRO", block_number, diff_min)
