...
def get_data():
    database = "zksync_data"
    conn = get_conn(database=database)
    cursor = conn.cursor()
    sql = "select block_number, timestamp from zksync_source_block order by block_number desc limit 1"
    info = {
        "block_number": 0,
        "create_time": 0
    }
...
def monitor():
    ...
    if int(diff_min) > THRESHOLD:
        alert(text)
        send_email_alert("zkSync", block_number, diff_min)
