...
def get_bsc_data():
    ...
    cursor = conn.cursor()
    sql = "select block_number, timestamp from avax_source_block order by block_number desc limit 1"
    info = {
        "block_number": 0,
        "create_time": 0
    }
...
def avax_monitor():
    ...
    if int(diff_min) > THRESHOLD:
        alert(text)
        send_email_alert("Avax", block_number, diff_min)
