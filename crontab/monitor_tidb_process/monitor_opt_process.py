...
def get_data():
    ...
    cursor = conn.cursor()
    sql = "select block_number, timestamp from opti_source_block order by block_number desc limit 1"
    info = {
        "block_number": 0,
        "create_time": 0
    }
...
def monitor():
    ...
    text =f"【Tidb 解析延迟告警】主链: Optimism\n已解析高度: {block_number}\n当前延迟约: {diff_min}分钟"
    ...
    if int(diff_min) > THRESHOLD:
        alert(text)
        send_email_alert("OPT", block_number, diff_min)
